...
cols_reg = c('flow','density','speed','small','heavy','truck_prop','co2','pm25','pm10',
             'co','no','no2','o3','humidity','temperature','WS_R','WD_R','sinWD','cosWD')
dummies <- dummyVars(no2~.,data = mydata2[,cols_reg])
train_dummies = predict(dummies,newdata = train[,cols_reg])
test_dummies = predict(dummies,newdata = test[,cols_reg])
print(dim(train_dummies));print(dim(test_dummies))

#or
x <- model.matrix(no2~.,train)[,-1]
y_train <- train$no2
x_test <- model.matrix(no2~.,test)[,-1]
y_test <- test$no2
...
lasso_reg <- cv.glmnet(x, y_train, alpha = 1, standardize = TRUE, nfolds = 5)
...
lasso <- glmnet(scale(x),y_train, alpha = 0)
