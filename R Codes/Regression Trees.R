...
str(mydata2)
mydata2=mydata2[,-c(15)]
mydata2 = as.numeric(mydata2)
...
ggpairs(mydata2)
library(corrplot)
a <- data.frame(cor(mydata2[9:14],mydata2[1:8]))
write.csv(a,file = "cor plot.csv")
corrplot(a, method = "number")
ggduo(mydata2, 9:14,1:8, types = list(continuous = loess_with_cor),showStrips = FALSE)+
  geom_label(a)+
  theme(strip.text = element_text(size = rel(0.95)))
...
scatter <- function(data, mapping,...){
  ggplot(data = data, mapping = mapping)+geom_point(alpha = 0.5,color = "#ffaa00")+
    geom_smooth(method = lm, se = FALSE,  color = "#0d58a6")
}

density <- function(data, mapping){
  ggplot(data = data, mapping = mapping) +
    geom_density(alpha = 0.5, fill = "#0d58a6")
}
mydata3 = mydata2[,-c(1:8)]
ggpairs(mydata3,
        lower = list(continuous = scatter), diag = list(continuous = density))+
  theme_bw()

mydata3 = mydata2[,-c(1,9:14)]
ggpairs(mydata3,
        lower = list(continuous = scatter), diag = list(continuous = density))+
  theme_bw()

ggduo(mydata2,columnsX = 9:14, columnsY = 1:8,
      types = list(continuous = scatter))+
  theme(strip.text = element_text(size = rel(0.95)))
...
tree.o3=tree(o3???.,mydata2 ,subset=index) 
summary (tree.o3)
plot(tree.o3)
...
tree.co2=tree(co2???.,mydata2 ,subset=index) 
summary (tree.co2)
plot(tree.co2)
...
tree.co=tree(co???.,mydata2 ,subset=index) 
summary (tree.co)
plot(tree.co)
...
tree.no=tree(no???.,mydata2 ,subset=index) 
summary (tree.no)
plot(tree.no)
...
tree.no2=tree(no2???.,mydata2 ,subset=index) 
summary (tree.no2)
plot(tree.no2)
...
tree.pm25=tree(pm25???.-pm10,mydata2 ,subset=index) 
summary (tree.pm25)
plot(tree.pm25)
